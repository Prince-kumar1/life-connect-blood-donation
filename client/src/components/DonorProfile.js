...
const DonorProfile = () => {
  ...
  const validatePhone = (phone, countryCode) => {
    ...
    const phoneRegex = new RegExp(`^\\d{${selectedCountry.digits}}$`);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validatePhone(formData.phone, formData.countryCode)) {
      ...
      setMessage(`Phone number must be exactly ${selectedCountry?.digits} digits for ${selectedCountry?.country}`);
      return;
    }
    ...
    try {
      ...
      const response = await axios.put('http://localhost:5000/api/donor/profile', submitData);
